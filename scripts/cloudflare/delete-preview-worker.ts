...
// 1) Remove the Workers custom domain (also drops the DNS record CF created).
const listRes = await cf(
  `/workers/domains?hostname=${encodeURIComponent(apiHost)}`,
);
if (listRes.ok) {
  const body = (await listRes.json()) as { result?: { id: string }[] };
  for (const domain of body.result ?? []) {
    const del = await cf(`/workers/domains/${domain.id}`, { method: "DELETE" });
    console.info(
...
const scriptRes = await cf(`/workers/scripts/${workerName}?force=true`, {
  method: "DELETE",
});
