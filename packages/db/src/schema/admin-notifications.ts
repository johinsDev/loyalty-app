 ...
 * attention"), the counterpart to the customer-facing `notification` feed.
 *
 * One row **per recipient**: the send job resolves the audience (role floor +
 * store assignment, see the `admin-notifications` feature's `resolveAudience`)
 ...
 *
 * `storeId` is null for org-wide alerts; the admin store switcher shows those
...
export const adminNotification = sqliteTable(
  "admin_notification",
  {
    ...
    /** Recipient — a staff `user`, not a customer. */
    userId: text("user_id")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }),
    /** Null = org-wide (visible in every store scope). */
    storeId: text("store_id").references(() => store.id, {
      onDelete: "set null",
    }),
    /** Alert key from the catalog (e.g. `points-adjusted`). */
