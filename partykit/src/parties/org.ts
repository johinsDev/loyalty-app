...
/**
 * Per-organization party. One room per `org:<orgId>`, shared by every
 ...
 *
 * Flows mirror the customer party: HMAC-signed POSTs in from the server,
...
export default class OrgParty implements Party.Server {
  ...
  static async onBeforeConnect(
    ...
    try {
      // The ticket is signed against the full `org:<id>` room name (see
      ...
      // user id, and the API only mints one for staff of THIS org.
      await verifyTicket(token, secret, `org:${lobby.id}`);
    } catch {
