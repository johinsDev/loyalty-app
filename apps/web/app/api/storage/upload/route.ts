...
/**
 * Receives presigned-URL uploads from the `local` + `memory` providers.
 * R2 uploads NEVER hit this — they go directly to Cloudflare.
 *
 * Flow:
 *   1. Read `?token=<jwt>` from the query
 *   2. Verify HMAC + expiry + that the token's mode is "put"
 *   3. Validate `Content-Length` ≤ token's `maxSize` (when set)
 *   4. Stream the body to `storage.disk(payload.disk).put(...)`
 *
 * Returns 204 on success, 4xx on validation failures.
 */
