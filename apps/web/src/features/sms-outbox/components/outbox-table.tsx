...
/**
 * Server component that fetches a page of `sms_outbox` rows and
 * renders the table + pagination. Mirrors the WhatsApp outbox table
 * but adds an "Encoding/segments" column since SMS segmentation has
...
export async function OutboxTable({ to, status, search, page, pageSize }: Props) {
  ...
  const { rows, total } = await api.smsOutbox.list({
    to: to || undefined,
    status,
    ...
    <Card>
      <CardContent className="p-0">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="w-[32%]">{t("colRecipient")}</TableHead>
              <TableHead>{t("colBody")}</TableHead>
              <TableHead className="w-[120px]">{t("colStatus")}</TableHead>
              <TableHead className="w-[100px]">{t("colSegments")}</TableHead>
