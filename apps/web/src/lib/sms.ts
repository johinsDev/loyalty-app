...
/**
 * Bootstrap for `@loyalty/sms` in the customer PWA. One module,
 * imports anywhere via `import { sms } from "@/lib/sms"`.
 *
 * Provider selection (default if `SMS_PROVIDER` is unset):
 *   - local dev:        log    (lines via `@loyalty/log`)
 *   - preview deploy:   outbox (rows in `sms_outbox`)
