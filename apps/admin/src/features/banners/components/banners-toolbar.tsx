...
import { STATE_VALUES } from "../list-params";

const SHALLOW = { shallow: false } as const;

/**
 * Client toolbar for the server banners table: debounced search + state facet +
 * created-date range + multi-sort + column visibility + list/grid toggle. Every
 * control writes the URL with `shallow:false`, so each change re-runs the server
 * render of the table hole. Column descriptors come from {@link BANNER_COLUMNS}.
 */
export function BannersToolbar() {
  ...
  const sortableCols = BANNER_COLUMNS.filter((c) => c.sortable).map((c) => ({
    id: c.id,
    label: t(c.labelKey),
  }));
  const hideableCols = BANNER_COLUMNS.filter((c) => c.hideable && (c.id !== "stores" || multiStore)).map(
    (c) => ({ id: c.id, label: t(c.labelKey) }),
  );
