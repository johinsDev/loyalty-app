...
/**
 * Pre-wizard gallery: pick a curated template (preseeded rule + copy + bg,
...
export function PromoGallery() {
  const t = useTranslations("Promotions.gallery");
  const router = useRouter();
  const trpc = useTRPC();
  const templates = useQuery(trpc.promociones.templates.queryOptions());
  ...
  async function start(templateKey?: string) {
    ...
    try {
      ...
      router.push({ pathname: "/promotions/[id]", params: { id: res.promo.id } });
    } catch {
        ...
        {(templates.data ?? []).map((tpl) => (
          ...
          >
            ...
            >
              ...
              <span className="relative z-10 mb-1.5 inline-flex w-fit items-center gap-1 rounded-full bg-white/25 px-2.5 py-0.5 text-xs font-extrabold backdrop-blur-sm">
