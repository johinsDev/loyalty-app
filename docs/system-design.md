...
**Brand / sizing conventions** (mobile-first): brand theme = T4 teal (`--primary` oklch ≈ `#1BAD9D`) + navy text + Fraunces display / Inter body (via `next/font`); `--radius` 0.875rem. **Default input height = 56px (`h-14`, `rounded-xl`)** — `Input` + `InputPhone`, mobile-friendly touch targets; `Button` has a `gradient` variant (teal→mint pill) for customer CTAs. Customer screens are full-bleed mobile with sticky bottom CTAs.
